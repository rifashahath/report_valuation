import { FileText, Calendar, Building2, MapPin, Loader2, Copy, Check, Download, ShieldCheck } from 'lucide-react';
...
export default function ReportDetailView({ reportId }: ReportDetailViewProps) {
    ...
    const handleApprove = async () => {
        if (!reportId) return;
        try {
            await reportsApi.updateReport(reportId, { status: 'approved' });
    ...
    const formatMarkdown = (text: string): string => {
        ...
        html = html.replace(/^## (.*$)/gim, '<h2 class="text-2xl font-bold mt-8 mb-4 border-b border-slate-100 dark:border-slate-800 pb-2">$1</h2>');
        ...
        // Lists
        html = html.replace(/^\* (.+)$/gim, '<li class="ml-6 mb-2 relative pl-2 before:content-[\'•\'] before:absolute before:left-[-1rem] before:text-slate-400">$1</li>');
        html = html.replace(/^- (.+)$/gim, '<li class="ml-6 mb-2 relative pl-2 before:content-[\'•\'] before:absolute before:left-[-1rem] before:text-slate-400">$1</li>');
        html = html.replace(/^\d+\. (.+)$/gim, '<li class="ml-6 mb-2 list-decimal">$1</li>');

        // Wrap consecutive lists properly - simplified regex approach
        // Note: Full markdown parsing is complex with regex. For production, use 'react-markdown' or 'remark'.
        // We will stick to basic replacements for now as originally implemented but styled better.

        // Code blocks
        html = html.replace(/```([\s\S]*?)```/g, '<pre class="bg-slate-900 text-slate-50 p-4 rounded-xl overflow-x-auto my-6 shadow-sm border border-slate-800 text-sm"><code>$1</code></pre>');

        // Inline code
        html = html.replace(/`(.+?)`/g, '<code class="bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 px-1.5 py-0.5 rounded text-sm font-mono border border-slate-200 dark:border-slate-700">$1</code>');

        // Links
        html = html.replace(/\[(.+?)\]\((.+?)\)/g, '<a href="$2" class="text-brand-600 hover:text-brand-700 underline underline-offset-2 decoration-brand-200" target="_blank" rel="noopener noreferrer">$1</a>');

        // Paragraphs - Negative lookahead to ensure we don't wrap tags
        html = html.replace(/^(?!<[h|u|o|l|p|d|b])(.+)$/gim, '<p class="mb-4 leading-relaxed text-slate-600 dark:text-slate-300">$1</p>');
            ...
            <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex-shrink-0">
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
                            <div className="flex items-center gap-1.5">
                                <Calendar size={14} />
                                <span>{formatDate(reportData.created_at) || 'Unknown Date'}</span>
                            </div>
                            {reportData.bank_name && (
                                <div className="flex items-center gap-1.5">
                                    <Building2 size={14} />
                                    <span>{reportData.bank_name}</span>
                                </div>
                            )}
                            {reportData.location && (
                                <div className="flex items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {reportData.status !== 'approved' && (
                            <button
                                ...
                                className="flex items-center gap-2 px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white rounded-lg transition-colors text-sm font-semibold shadow-sm shadow-brand-200 dark:shadow-none"
                            >
                                <ShieldCheck size={16} />
                                Approve
                            </button>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                        ...
                        item.value ? (
                            <div key={i} className="px-3 py-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-800">
            ...
            <div className="flex-1 overflow-y-auto px-8 py-8 custom-scrollbar">
                {analysisResult ? (
                    <div className="max-w-4xl mx-auto">
                        <div className="prose prose-slate dark:prose-invert prose-headings:font-bold prose-h1:text-3xl prose-h2:text-2xl prose-h2:border-b prose-h2:border-slate-100 dark:prose-h2:border-slate-800 prose-h2:pb-2 prose-h2:mt-8 prose-p:text-slate-600 dark:prose-p:text-slate-300 prose-p:leading-relaxed prose-li:text-slate-600 dark:prose-li:text-slate-300 prose-strong:text-slate-800 dark:prose-strong:text-slate-200 max-w-none"
